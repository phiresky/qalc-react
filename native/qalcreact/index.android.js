			...
			<ScrollView>
				{reverseMap(this.props.calculations, ({input, output}, i) => 
					<TouchableHighlight key={i}
						onPress={() => this.props.onCalculationPressed({input, output})}>
						<View style={styles.calculation}>
							<Text style={styles.calculationInput}>{input}</Text>
							<Text style={styles.calculationOutput}>{""}{output}</Text>
						</View>
					</TouchableHighlight>
				)}
			</ScrollView>
...
class QalcGui extends Component {
	constructor() {
		...
		presetLines.map(input => qalculate(input).then(output => this.addLine({input, output})));
	}
	addLine(line) {
	...
	qalc() {
		...
		qalculate(input)
		.then(output => this.addLine({input, output}))
		this.setState({text: ""});
	}
	render() {
		return (
			<View style={styles.container}>
				...
				<QalcHistory
					...
					onCalculationPressed={({input}) => this.setState({text:input})}
				/>
			</View>
		);
	}
