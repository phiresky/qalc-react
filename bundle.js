...
$__System.register("1", ["28", "29", "a0", "a3", "a2"], function($__export) {
  "use strict";
  var React,
      lzString,
      ReactDOM,
  ...
  function loadPresetLines() {
    ...
    presets.map((input) => QalcLib.qalculate(input).then((output) => guiInst.addLine(new GuiLineElement(input, output))).catch((error) => guiInst.addLine(new GuiLineElement(input, new TaggedString("" + error)))));
  }
  return {
      ...
      GUI = class GUI extends React.Component {
        ...
        onSubmit(evt) {
          ...
          if (input.trim().length > 0)
            QalcLib.qalculate(input).then((output) => this.addLine(new GuiLineElement(input, output))).catch((reason) => this.addLine(new GuiLineElement(input, new TaggedString("" + reason))));
          this.setState({
            currentInput: "",
            currentOutput: new TaggedString()
          });
        ...
        setInput(input) {
          ...
          if (/[=≈]/.test(input))
            ...
            QalcLib.qalculate(input).then((output) => this.setState({currentOutput: output})).catch((reason) => this.setState({currentOutput: new TaggedString("" + reason)}));
        }
        onChange(evt) {
